...
struct StoryRabbitPlayerView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            NowPlayingQueueView(selectedTour: $selectedTour, selectedPOI: $selectedPOI, currentPOIs:$currentPOIs)
            .task {
                if let tour = selectedTour, let firstPOI = currentPOIs.filter({$0.tour_id == tour.id}).first {
                    selectedPOI = firstPOI
                }
            }
            .onChange(of: currentPOIs) { oldValue, newValue in

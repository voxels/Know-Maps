...
final class MockChatResultViewModel: ChatResultViewModel {
    
    // Variables to hold mock data
    var mockCurrentLocation: CLLocation?
    var mockPlaceResults: [ChatResult] = []
    var mockFilteredPlaceResults: [ChatResult] = []
    var mockRecommendedPlaceResults: [ChatResult] = []
    var mockRelatedPlaceResults: [ChatResult] = []
    var mockCachedLocationResults: [LocationResult] = []
    var mockTasteResults: [CategoryResult] = []
    var mockSelectedPlaceChatResult: ChatResult.ID?
    var mockIsFetchingPlaceDescription: Bool = false
    var mockAutocompleteResult: [String: Any] = [:]
    var mockDetailsResult: PlaceDetailsResponse?
    var mockFetchRelatedPlacesResult: [RecommendedPlaceSearchResponse] = []
    
    
    // Override: mock for fetching current location name
    override public func currentLocationName() async throws -> String? {
        if let location = mockCurrentLocation {
            return "\(location.coordinate.latitude), \(location.coordinate.longitude)"
        }
        return "Mock Location"
    }
    
    // Override: mock for filtered place results
    override public var filteredPlaceResults: [ChatResult] {
        get {
            return !mockFilteredPlaceResults.isEmpty ? mockFilteredPlaceResults : mockPlaceResults
        }
    }
    
    // Override: mock for recommended place results
    override public var recommendedPlaceResults: [ChatResult] {
        get {
            return mockRecommendedPlaceResults
        }
        set {
            mockRecommendedPlaceResults = newValue
        }
    }
    
    // Override: mock for related place results
    override public var relatedPlaceResults: [ChatResult] {
        get {
            return mockRelatedPlaceResults
        }
        set {
            mockRelatedPlaceResults = newValue
        }
    }
    
    // Override: mock for refreshing the cache
    override public func refreshCache(cloudCache: CloudCache) async throws {
        // Simulate cache refreshing
        isRefreshingCache = false
    }
    
    // Override: mock for resetting the place model
    override public func resetPlaceModel() {
    ...
    // Override: mock for retrieving place chat result by FSQ ID
    override public func placeChatResult(for selectedPlaceFsqID: String) -> ChatResult? {
        return mockPlaceResults.first(where: { $0.placeResponse?.fsqID == selectedPlaceFsqID })
    }
    
    // Override: mock for retrieving place chat result by ID
    override public func placeChatResult(for selectedChatResultID: ChatResult.ID) -> ChatResult? {
        return mockPlaceResults.first(where: { $0.id == selectedChatResultID })
    }
    
    // Override: mock for retrieving FSQ user
    override public func retrieveFsqUser() async throws -> Bool {
        return true
    }
    
    // Override: mock for refreshing cached locations
    override public func refreshCachedLocations(cloudCache: CloudCache) async throws {
        // Simulate refreshing cached locations
        cachedLocationResults = mockCachedLocationResults
    }
    
    // Override: mock for refreshing cached categories
    override public func refreshCachedCategories(cloudCache: CloudCache) async throws {
        // Simulate refreshing cached categories
    }
    
    // Override: mock for refreshing cached tastes
    override public func refreshCachedTastes(cloudCache: CloudCache) async throws {
        // Simulate refreshing cached tastes
        tasteResults = mockTasteResults
    }
    
    // Override: mock for appending cached location
    override public func appendCachedLocation(with record: UserCachedRecord) {
        // Simulate appending a cached location
        let location = LocationResult(locationName: record.title, location: CLLocation(latitude: 0, longitude: 0))
        cachedLocationResults.append(location)
    }
    
    // Override: mock for appending cached category
    override public func appendCachedCategory(with record: UserCachedRecord) {
        // Simulate appending a cached category
    }
    
    // Override: mock for appending cached taste
    override public func appendCachedTaste(with record: UserCachedRecord) {
        // Simulate appending a cached taste
    }
    
    // Override: mock for appending cached list
    override public func appendCachedList(with record: UserCachedRecord) {
        // Simulate appending a cached list
    }
    
    // Override: mock for appending cached place
    override public func appendCachedPlace(with record: UserCachedRecord) {
        // Simulate appending a cached place
    }
    
    // Override: mock for checking if categories are cached
    override public func cachedCategories(contains category: String) -> Bool {
        return true
    }
    
    // Override: mock for checking if tastes are cached
    override public func cachedTastes(contains taste: String) -> Bool {
        return true
    }
    
    // Override: mock for checking if location is cached
    override public func cachedLocation(contains location: String) -> Bool {
        return cachedLocationResults.contains { $0.locationName == location }
    }
    
    // Override: mock for fetching location chat result by title
    override public func locationChatResult(with title: String) async -> LocationResult {
        return LocationResult(locationName: title, location: mockCurrentLocation)
    }
    
    // Override: mock for searching intents
    override public func searchIntent(intent: AssistiveChatHostIntent, location: CLLocation?) async throws {
        if intent.intent == .Place {
            let response = PlaceSearchResponse(fsqID: "mock-fsq-id", name: "Mock Place", categories: [], latitude: 0, longitude: 0, address: "", addressExtended: "", country: "", dma: "", formattedAddress: "", locality: "", postCode: "", region: "", chains: [], link: "", childIDs: [], parentIDs: [])
            mockPlaceResults = [ChatResult(title: response.name, placeResponse: response, recommendedPlaceResponse: nil)]
            placeResults = mockPlaceResults
        } else if intent.intent == .AutocompleteSearch {
            let autocompleteResult = ["autocomplete": "test"]
            mockAutocompleteResult = autocompleteResult
            mockPlaceResults = [ChatResult(title: "test", placeResponse: nil, recommendedPlaceResponse: nil)]
            placeResults = mockPlaceResults
        } else if intent.intent == .AutocompleteTastes {
            mockTasteResults = [CategoryResult(parentCategory: "Spicy", categoricalChatResults: []), CategoryResult(parentCategory: "Cheesy", categoricalChatResults: [])]
            tasteResults = mockTasteResults
        }
    }
    
    // Override: mock for detail intent
    override public func detailIntent(intent: AssistiveChatHostIntent) async throws {
        if let response = intent.selectedPlaceSearchResponse {
            let details = PlaceDetailsResponse(searchResponse: response, photoResponses: nil, tipsResponses: nil, description: "Mock Details", tel: nil, fax: nil, email: nil, website: nil, socialMedia: nil, verified: nil, hours: nil, openNow: nil, hoursPopular: nil, rating: 1.0, stats: nil, popularity: 1, price: nil, menu: nil, dateClosed: nil, tastes: nil, features: nil)
            mockDetailsResult = details
            // Update placeResults with details
            if let index = mockPlaceResults.firstIndex(where: { $0.placeResponse?.fsqID == response.fsqID }) {
                var updatedResult = mockPlaceResults[index]
                updatedResult.replaceDetails(response: details)
                mockPlaceResults[index] = updatedResult
                placeResults = mockPlaceResults
            }
        }
    }
    
    // Override: mock for autocomplete place model
    override public func autocompletePlaceModel(caption: String, intent: AssistiveChatHostIntent, location: CLLocation) async throws {
        mockPlaceResults = [ChatResult(title: "Pizza Place", placeResponse: nil, recommendedPlaceResponse: nil)]
        placeResults = mockPlaceResults
    }
    
    // Override: mock for refreshing tastes
    override public func refreshTastes(page: Int) async throws {
        mockTasteResults = [CategoryResult(parentCategory: "Sour", categoricalChatResults: []), CategoryResult(parentCategory: "Sweet", categoricalChatResults: [])]
        tasteResults = mockTasteResults
    }
    
    // Override: mock for autocomplete tastes
    override public func autocompleteTastes(lastIntent: AssistiveChatHostIntent) async throws {
        mockTasteResults = [CategoryResult(parentCategory: "Spicy", categoricalChatResults: []), CategoryResult(parentCategory: "Cheesy", categoricalChatResults: [])]
        tasteResults = mockTasteResults
    }
    
    // Override: mock for fetching related places
    override public func fetchRelatedPlaces(for fsqID: String) async throws -> [RecommendedPlaceSearchResponse] {
        let relatedPlace = RecommendedPlaceSearchResponse(fsqID: "mock-fsq-id", name: "Test Related Place", categories: [], latitude: 0, longitude: 0, neighborhood: "", address: "", country: "", city: "", state: "", postCode: "", formattedAddress: "", photo: nil, photos: [], tastes: [])
        return [relatedPlace]
    }
    
    // Override: mock for fetching details
    override public func fetchDetails(for responses: [PlaceSearchResponse]) async throws -> [PlaceDetailsResponse] {
        if let details = mockDetailsResult {
            return [details]
        } else {
...
//    // Override: mock for streaming result
//    override public func didReceiveStreamingResult(with string: String, for result: ChatResult, promptTokens: Int, completionTokens: Int) async {
//        if let index = placeResults.firstIndex(where: { $0.id == result.id }) {
//            var updatedResult = placeResults[index]
//            if let existingDescription = updatedResult.placeDetailsResponse?.description {
//                updatedResult.placeDetailsResponse?.description = existingDescription + string
//            } else {
//                updatedResult.placeDetailsResponse?.description = string
//            }
//            placeResults[index] = updatedResult
//        }
//        isFetchingPlaceDescription = false
//    }
//    
//    // Override: mock for beginning streaming result
//    override public func willReceiveStreamingResult(for chatResultID: ChatResult.ID) async {
//        isFetchingPlaceDescription = true
//    }
//    
//    // Override: mock for finishing streaming result
//    override public func didFinishStreamingResult() async {
//        isFetchingPlaceDescription = false
//    }
}
...
final class ChatResultViewModelTests: XCTestCase {
    ...
    var mockViewModel: MockChatResultViewModel!
    ...
    override func setUp() {
        ...
        mockPersonalizedSearchSession = MockPersonalizedSearchSession(cloudCache: MockCloudCache())
        mockLocationProvider = MockLocationProvider()
        mockCloudCache = MockCloudCache()
        mockFeatureFlags = MockFeatureFlags()
        
        mockViewModel = MockChatResultViewModel(
            locationProvider: mockLocationProvider,
            cloudCache: mockCloudCache,
            featureFlags: mockFeatureFlags
        )
        
        // Injecting the mock sessions
        mockViewModel.placeSearchSession = mockPlaceSearchSession
        mockViewModel.personalizedSearchSession = mockPersonalizedSearchSession
    }
    
    func testCurrentLocationName() async throws {
        // Arrange
        let mockLocation = CLLocation(latitude: 40.7128, longitude: -74.0060)
        mockViewModel.mockCurrentLocation = mockLocation
        
        // Act
        let locationName = try await mockViewModel.currentLocationName()
        
        // Assert
        XCTAssertEqual(locationName, "40.7128, -74.006")
    }
    
    func testFilteredPlaceResults() {
        // Arrange
        let mockChatResult = ChatResult(title: "Test Place", placeResponse: nil, recommendedPlaceResponse: nil)
        mockViewModel.mockPlaceResults = [mockChatResult]
        
        // Act
        let filteredResults = mockViewModel.filteredPlaceResults
        
        // Assert
        XCTAssertEqual(filteredResults.count, 1)
        XCTAssertEqual(filteredResults.first?.title, "Test Place")
    }
    
    func testRefreshCache() async throws {
        // Act
        try await mockViewModel.refreshCache(cloudCache: mockCloudCache)
        
        // Assert
        XCTAssertFalse(mockViewModel.isRefreshingCache)
    }
    
    func testPlaceChatResultForFsqID() {
        // Arrange
        let fsqID = "mock-fsq-id"
        let mockChatResult = ChatResult(
            title: "Test Place",
        ...
        // Act
        let result = mockViewModel.placeChatResult(for: fsqID)
    ...
    @MainActor func testResetPlaceModel() {
        // Act
        mockViewModel.resetPlaceModel()
        
        // Assert
        XCTAssertNil(mockViewModel.selectedPlaceChatResult)
        XCTAssertTrue(mockViewModel.placeResults.isEmpty)
        XCTAssertTrue(mockViewModel.recommendedPlaceResults.isEmpty)
        XCTAssertTrue(mockViewModel.relatedPlaceResults.isEmpty)
    }
    
    func testAutocompleteTastes() async throws {
        // Arrange
        let mockIntent = AssistiveChatHostIntent(
            caption: "Pizza",
            intent: .AutocompleteTastes,
            ...
            selectedDestinationLocationID: nil,
            placeDetailsResponses: nil,
            queryParameters: nil
        )
        let mockTastes = ["Spicy", "Cheesy"]
        mockPersonalizedSearchSession.mockAutocompleteResult = ["tastes": mockTastes]
        
        // Act
        try await mockViewModel.autocompleteTastes(lastIntent: mockIntent)
        
        // Assert
        XCTAssertEqual(mockViewModel.tasteResults.map { $0.parentCategory }, mockTastes)
    }
    
    func testRefreshTastes() async throws {
        // Arrange
        mockPersonalizedSearchSession.mockTastesResult = ["Sour", "Sweet"]
        
        // Act
        try await mockViewModel.refreshTastes(page: 1)
        
        // Assert
        XCTAssertEqual(mockViewModel.tasteResults.map { $0.parentCategory }, ["Sour", "Sweet"])
    }
    
    func testSearchIntentPlace() async throws {
        // Arrange
        let mockIntent = AssistiveChatHostIntent(
            caption: "Coffee",
            intent: .Place,
        ...
        let mockResponse = PlaceSearchResponse(
            fsqID: "mock-fsq-id",
        ...
        mockPlaceSearchSession.mockQueryResult = ["results": [mockResponse]]
        
        // Act
        try await mockViewModel.searchIntent(intent: mockIntent, location: CLLocation(latitude: 40.7128, longitude: -74.0060))
        
        // Assert
        XCTAssertTrue(mockPlaceSearchSession.queryCalled)
        XCTAssertEqual(mockViewModel.placeResults.first?.title, "Test Coffee Place")
    }
    
    func testSearchIntentAutocomplete() async throws {
        // Arrange
        let mockIntent = AssistiveChatHostIntent(
            caption: "Pizza",
            intent: .AutocompleteSearch,
            selectedPlaceSearchResponse: nil,
            selectedPlaceSearchDetails: nil,
            selectedRecommendedPlaceSearchResponse: nil,
            placeSearchResponses: [],
        ...
        let mockResponse = ["autocomplete": "test"]
        mockPersonalizedSearchSession.mockAutocompleteResult = mockResponse
        
        // Act
        try await mockViewModel.searchIntent(intent: mockIntent, location: CLLocation(latitude: 40.7128, longitude: -74.0060))
        
        // Assert
        XCTAssertTrue(mockPersonalizedSearchSession.autocompleteCalled)
        XCTAssertEqual(mockViewModel.placeResults.first?.title, "test")
    }
    
    func testFetchDetailsForPlaces() async throws {
        // Arrange
        let mockPlaceResponse = PlaceSearchResponse(
            fsqID: "mock-fsq-id",
            name: "Test Place",
            categories: [],
            latitude: 40.7128,
        ...
        mockPlaceSearchSession.mockDetailsResult = PlaceDetailsResponse(
            searchResponse: mockPlaceResponse,
            photoResponses: nil,
            tipsResponses: nil,
            description: nil,
            tel: nil,
        ...
        // Act
        let details = try await mockViewModel.fetchDetails(for: [mockPlaceResponse])
        
        // Assert
        XCTAssertEqual(details.first?.searchResponse.fsqID, "mock-fsq-id")
        XCTAssertTrue(mockPlaceSearchSession.detailsCalled)
    }
    
    func testResetAndCacheResults() async throws {
        // Arrange
        let mockCachedRecord = UserCachedRecord(
        ...
        mockCloudCache.mockFetchedRecords = [mockCachedRecord]
        
        // Act
        try await mockViewModel.refreshCachedLocations(cloudCache: mockCloudCache)
        
        // Assert
        XCTAssertEqual(mockViewModel.cachedLocationResults.first?.locationName, "MockTitle")
    }
    ...
    func testAutocompletePlaces() async throws {
        ...
        let mockResponse = ["autocomplete": "Pizza Place"]
        mockPlaceSearchSession.mockAutocompleteResult = mockResponse
        
        // Act
        try await mockViewModel.autocompletePlaceModel(caption: "Pizza", intent: mockIntent, location: CLLocation(latitude: 40.7128, longitude: -74.0060))
        
        // Assert
        XCTAssertEqual(mockViewModel.placeResults.first?.title, "Pizza Place")
    }
    
    func testRelatedPlaces() async throws {
        // Arrange
        mockPersonalizedSearchSession.mockRelatedVenuesResult = ["relatedVenue": "Test Related Place"]
        
        // Act
        let relatedPlaces = try await mockViewModel.fetchRelatedPlaces(for: "mock-fsq-id")
        
        // Assert
        XCTAssertEqual(relatedPlaces.first?.fsqID, "mock-fsq-id")
    }
    
//    func testStreamResult() async throws {
//        // Arrange
//        let mockChatResult = ChatResult(title: "Test Place", placeResponse: nil, recommendedPlaceResponse: nil)
//        mockViewModel.placeResults = [mockChatResult]
//        
//        // Act
//        await mockViewModel.didReceiveStreamingResult(with: "Updated Description", for: mockChatResult, promptTokens: 10, completionTokens: 5)
//        
//        // Assert
//        XCTAssertEqual(mockViewModel.placeResults.first?.placeDetailsResponse?.description, "Updated Description")
//    }
}
